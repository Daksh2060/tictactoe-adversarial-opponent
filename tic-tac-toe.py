...
def create_frames(root):
    ...
    #Changed to fix issue with changing depth
...
def on_click(button):
    ...
    # used custom calls to search methods with the cutoff depth
    try:
        choice = choices.get()
        if "Random" in choice:
            ...
            if gdepth == -1:
                ...
                a, b = expect_minmax_cutoff(gBoard, state2, gdepth)

        elif "MinMax" in choice:
            if gdepth == -1:
                a, b = minmax(gBoard, state2)
            else:
                a, b = minmax_cutoff(gBoard, state2, gdepth)
        else:
            if gdepth == -1:
                a, b = alpha_beta_search(gBoard, state2)
            else:
                a, b = alpha_beta_cutoff_search(gBoard, state2, gdepth)
